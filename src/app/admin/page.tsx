    ...
    <AdminLayout>
      <div className="space-y-8 md:space-y-12">
        <div className="space-y-2">
          <h1 className="text-2xl md:text-4xl font-serif uppercase tracking-tight">Dashboard Overview</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8">
          {stats.map((stat) => {
            ...
            return (
              <div key={stat.name} className="bg-black border border-white/5 p-6 md:p-8 rounded-sm space-y-6 relative overflow-hidden group">
        ...
        <div className="space-y-4 md:space-y-6 pt-4 md:pt-8">
          <h2 className="text-lg md:text-xl font-serif uppercase tracking-widest border-b border-white/5 pb-4">Recent Orders</h2>
            ...
            <table className="w-full text-left border-collapse min-w-[800px]">
              <thead>
                <tr className="border-b border-white/10 text-[10px] uppercase tracking-widest text-zinc-500 bg-zinc-950">
                  <th className="p-4 md:p-6 font-normal">Order ID</th>
                  <th className="p-4 md:p-6 font-normal">Customer</th>
                  <th className="p-4 md:p-6 font-normal">Status</th>
                  <th className="p-4 md:p-6 font-normal">Amount</th>
                  <th className="p-4 md:p-6 font-normal">Date</th>
                </tr>
              </thead>
                ...
                {loading ? (
                  ...
                  recentOrders.map((order) => (
                    <tr key={order.id} className="border-b border-white/5 hover:bg-white/5 transition-colors group">
                      <td className="p-4 md:p-6 text-zinc-400 font-mono" title={order.id}>#{order.id.split('-')[0]}</td>
                      <td className="p-4 md:p-6">{order.shipping_address?.name || 'Unknown'}</td>
                      ...
                      <td className="p-4 md:p-6 font-mono text-[11px]">₹{order.total_amount}</td>
                      <td className="p-4 md:p-6 text-zinc-500">{new Date(order.created_at).toLocaleDateString()}</td>
                    </tr>
                  ))
                )}
              </tbody>
